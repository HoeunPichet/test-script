...
input double   MaxOpenRisk = 6.0;                    // Max Total Open Risk (%) - Higher for M1
input int      MaxConcurrentTrades = 5;              // Max Concurrent Trades - More for M1 scalping
input double   MinRiskRewardRatio = 1.3;             // Min Risk:Reward Ratio - Realistic for M1

//=== ENTRY STRATEGY ===
input group "=== MOMENTUM ENTRY SETTINGS ==="
input bool     UseMultiConfirmation = true;          // Use Multi-Timeframe Confirmation
input int      MomentumPeriod = 8;                   // Momentum Period (bars) - Optimized for M1
input double   MinMomentumStrength = 50.0;           // Min Momentum Strength (0-100) - Lower for M1
...
input int      TrendMA_Fast = 10;                    // Fast MA Period - Faster for M1
input int      TrendMA_Slow = 30;                    // Slow MA Period - Faster for M1
input ENUM_TIMEFRAMES TrendTimeframe = PERIOD_M5;    // Trend Timeframe - M5 for M1 trading

//=== ADX FILTER (Trend Strength) ===
input group "=== ADX FILTER ==="
input bool     UseADXFilter = true;                  // Enable ADX Filter
input int      ADXPeriod = 10;                       // ADX Period - Faster for M1
input double   MinADX = 15.0;                        // Min ADX Value - Lower for M1 trends

//=== RSI FILTER (Overbought/Oversold) ===
input group "=== RSI FILTER ==="
input bool     UseRSIFilter = true;                  // Enable RSI Filter
input int      RSIPeriod = 10;                       // RSI Period - Faster for M1
input double   RSI_Overbought = 75.0;                // RSI Overbought Level - Relaxed for M1
input double   RSI_Oversold = 25.0;                  // RSI Oversold Level - Relaxed for M1
...
input double   FixedSL_Pips = 10.0;                  // Fixed SL (pips) - Tighter for M1
input int      ATR_Period = 10;                      // ATR Period for SL/TP - Faster for M1
input double   ATR_SL_Multiplier = 1.5;              // ATR Multiplier for SL - Tighter for M1
input double   ATR_TP_Multiplier = 2.5;              // ATR Multiplier for TP - Scalping target
input int      SwingLookback = 10;                   // Swing High/Low Lookback - Shorter for M1

//=== TRAILING STOP ===
input group "=== TRAILING STOP ==="
input bool     UseTrailingStop = true;               // Enable Trailing Stop
input double   TrailingStart_Pips = 8.0;             // Start Trailing at (pips profit) - Quick for M1
input double   TrailingStop_Pips = 6.0;              // Trailing Stop Distance (pips) - Tight for M1
input double   TrailingStep_Pips = 2.0;              // Trailing Step (pips) - Small steps for M1

//=== BREAK EVEN ===
input group "=== BREAK EVEN ==="
input bool     UseBreakEven = true;                  // Move to Break Even
input double   BreakEven_Pips = 6.0;                 // Activate at profit (pips) - Quick for M1
input double   BreakEven_Plus = 1.0;                 // Lock profit (pips) - Small for M1
...
input double   MaxSpreadPips = 2.0;                  // Max Spread (pips) - Very tight for M1 scalping
